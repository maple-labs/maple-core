...
describe("Maple Token", function () {
  ...
  it("transferFrom() functionality", async function () {
    ...
    expect(
      await mapleTokenExternal.transferFrom(
        governor,
        accounts[1],
  ...
  it("FDT: mint fundsToken, updateFunds() ", async function () {
    ...
    const accounts = await ethers.provider.listAccounts();
    const withdrawableFundsOfGovernor = await mapleToken.withdrawableFundsOf(
      governor
    );
    const accumulativeFundsOfGovernor = await mapleToken.accumulativeFundsOf(
      governor
    );
    const withdrawableFundsOfAccountOne = await mapleToken.withdrawableFundsOf(
      accounts[1]
    );
    const accumulativeFundsOfAccountOne = await mapleToken.accumulativeFundsOf(
      accounts[1]
    );
    ...
    const mapleTokenBalanceAccountOne = await mapleToken.accumulativeFundsOf(
      accounts[1]
    );

    const expectedWithdrawGovernor =
      (pointsPerShare * mapleTokenBalanceGovernor) /
      pointsMultiplier /
      10 ** mapleTokenDecimals;
    const expectedWithdrawAccountOne =
      (pointsPerShare * mapleTokenBalanceAccountOne) /
      pointsMultiplier /
      10 ** mapleTokenDecimals;
  ...
  it("FDT: withdrawFunds() ", async function () {
    ...
    const withdrawnFundsOfGovernor = await mapleToken.withdrawnFundsOf(
      governor
    );
    const withdrawableFundsOfGovernor = await mapleToken.withdrawableFundsOf(
      governor
    ...
    const withdrawnFundsOfAccountOne = await mapleToken.withdrawnFundsOf(
      accounts[1]
    );
    const withdrawableFundsOfAccountOne = await mapleToken.withdrawableFundsOf(
      accounts[1]
