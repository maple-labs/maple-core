...
contract LoanVaultTest is TestUtil {
    ...
    function setUp() public {
        ...
        globals.addCalculator(address(bulletCalc));
	globals.addCalculator(address(lateFeeCalc));
	globals.addCalculator(address(premiumCalc));
    ...
    function test_createLoanVault() public {
        ...
        address[3] memory calculators = [address(bulletCalc), address(lateFeeCalc), address(premiumCalc)];

        LoanVault loanVault =
            ali.createLoanVault(loanVaultFactory, DAI, WETH, specifications, calculators);
