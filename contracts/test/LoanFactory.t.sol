...
contract LoanFactoryTest is TestUtil {
    ...
    function test_createLoan_invalid_locker_factories() public {
        ...
        uint256[6] memory specs = [10, 10, 2, 10_000_000 * USD, 30, 5];
    ...
    function test_createLoan_invalid_assets_and_specs() public {
        ...
        assertTrue(!bob.try_createLoan(address(loanFactory), USDC, WETH, address(flFactory), address(clFactory), [10, 10, 0, 10_000_000 * USD, 30, 5], calcs));
        assertEq(loanFactory.loansCreated(), 0, "Colluded state");  // Should be 0.

        // Fails because of error - ERR_INVALID_TERM_AND_PAYMENT_INTERVAL_DIVISION
        assertTrue(!bob.try_createLoan(address(loanFactory), USDC, WETH, address(flFactory), address(clFactory), [10, 19, 2, 10_000_000 * USD, 30, 5], calcs));
        assertEq(loanFactory.loansCreated(), 0, "Colluded state");  // Should be 0.

        // Fails because of error - ERR_REQUEST_AMT_EQUALS_ZERO
        assertTrue(!bob.try_createLoan(address(loanFactory), USDC, WETH, address(flFactory), address(clFactory), [uint256(10), 10, 2, uint256(0), 30, 5], calcs));
        assertEq(loanFactory.loansCreated(), 0, "Colluded state");  // Should be 0.

        // fails because of error - ERR_FUNDING_PERIOD_EQUALS_ZERO
        assertTrue(!bob.try_createLoan(address(loanFactory), USDC, WETH, address(flFactory), address(clFactory), [10, 10, 2, 10_000_000 * USD, 30, 0], calcs));
        assertEq(loanFactory.loansCreated(), 0, "Colluded state");  // Should be 0.
