...
contract Loan is ILoan, LoanFDT, Pausable {
    ...
    function drawdown(uint256 amt) external virtual override {
        ...
        uint256 _feePaid = feePaid = amt.mul(globals.investorFee()).div(10_000);  // Update fees paid for `claim()`.
        uint256 treasuryAmt        = amt.mul(globals.treasuryFee()).div(10_000);  // Calculate amount to send to the MapleTreasury.
    ...
    function _drawdown(uint256 amt, uint256 _feePaid, uint256 treasuryAmt) internal {
        ...
        address treasury = _globals(superFactory).mapleTreasury();
