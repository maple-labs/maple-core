...
describe("Borrower Journey", function () {
    ...
    const getNextPaymentAmount = (
      ...
      if (interestStructure === "BULLET") {
        let interest = BigNumber.from(principalOwed)
          .mul(APR)
          .mul(repaymentFrequencyDays)
          .div(365)
          .div(10000);
        return paymentsRemaining == 1
          ? [interest.add(principalOwed), interest, principalOwed]
          : [interest, 0, interest];
  ...
  it("E - Fetch important Loan information", async function () {
    ...
    const SECONDS_REMAINING_FUNDING_PERIOD =
      LOAN_FUNDING_ENDS - Date.now() / 1000;
  ...
  xit("F - Fetch collateral required for drawdown, facilitate approve() calls", async function () {
    ...
    const BORROWER_ADDRESS = await Loan.borrower({ gasLimit: 6000000 });
